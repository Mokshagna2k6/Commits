      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge variant={selected.state === 'ACTIVE' ? 'success' : 'neutral'}>{selected.state}</Badge>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><span className="text-warm-500">Model</span><p className="font-medium">{selected.model}</p></div>
            <div><span className="text-warm-500">Tier</span><p className="font-medium">{selected.tier}</p></div>
            <div><span className="text-warm-500">Start</span><p className="font-medium">{formatDate(selected.startDate)}</p></div>
            <div><span className="text-warm-500">End</span><p className="font-medium">{selected.endDate ? formatDate(selected.endDate) : '–'}</p></div>
          </div>
        </div>
      </div>
    );
          ...
          {engagements.map(eng => (
            <Link key={eng.id} to={`/app/client/engagements/${eng.id}`}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-medium text-warm-900">{eng.model} – {eng.tier}</p>
                  <p className="text-sm text-warm-500">{eng.id}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Badge variant={eng.state === 'ACTIVE' ? 'success' : 'neutral'}>{eng.state}</Badge>

...
export default function Engagements() {
  ...
  const filtered = filter === 'all' ? engagements : engagements.filter(e => e.state === filter);
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="text-warm-500 text-left bg-warm-50">
                ...
                <th className="px-5 py-3">Tier</th><th className="px-5 py-3">Org</th>
                <th className="px-5 py-3">State</th><th className="px-5 py-3">Start</th>
              </tr></thead>
              <tbody>
                {filtered.map(e => (
                  <tr key={e.id} className="border-t border-warm-50 hover:bg-warm-50/50">
                    ...
                    <td className="px-5 py-3">{e.tier}</td>
                    <td className="px-5 py-3">{e.orgId || '–'}</td>
                    <td className="px-5 py-3"><Badge variant={e.state === 'ACTIVE' ? 'success' : 'neutral'}>{e.state}</Badge></td>
                    <td className="px-5 py-3 text-warm-500">{formatDate(e.startDate)}</td>
                  </tr>
                ))}
              </tbody>
            </table>

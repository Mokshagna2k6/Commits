...
// Tier-adaptive checkout (Product Bible §5.1). Same state machine underneath
// (one quote, one step counter) — the tier only controls which steps and
// fields are shown, per the Starter/Growth/Premium step tables in the Bible.
const STEP_NAMES = {
  STARTER: ['Confirm Package', 'Your Details', 'Pay'],
  GROWTH: ['Review Scope', 'Account', 'Project Setup', 'Payment Terms', 'Docs & Pay'],
  PREMIUM: ['Review Scope', 'Organisation', 'Engagement', 'Payment Terms', 'Docs & E-Sign', 'Pay', 'Confirm'],
};

const CONTRACT_STACK = {
  STARTER: ['2-page Service Agreement (Hindi + English)'],
  GROWTH: ['SOW (Statement of Work)', 'MSA (simplified)'],
  PREMIUM: ['SOW', 'MSA', 'NDA', 'IP Assignment Deed', 'DPA', 'SLA Addendum'],
};
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-4">
      ...
      <div className="bg-white rounded-3xl border border-warm-200 p-6 md:p-8 space-y-6">
        {/* Step 0: Review Scope / Confirm Package (all tiers) */}
        {step === 0 && (
        ...
        {/* Account / Your Details / Organisation step */}
        ...
        {/* Docs & Pay (Growth — accept + pay together) / Docs & E-Sign (Premium — accept, then a separate Pay step) */}
        {(steps[step] === 'Docs & Pay' || steps[step] === 'Docs & E-Sign') && (
          <div className="space-y-4">
            <h2 className="font-bold text-warm-900">{steps[step]}</h2>
            ...
            <label className="flex items-start gap-3 text-sm text-warm-700 cursor-pointer">
              ...
              <span>I've reviewed and accept the {tier === 'PREMIUM' ? 'full document suite' : 'agreement'} above. {tier === 'PREMIUM' ? 'Aadhaar e-sign follows for orders ≥₹5L.' : 'This acceptance is recorded with your phone/email as evidence.'}</span>
            </label>
            {steps[step] === 'Docs & Pay' ? (
              <Button variant="primary" className="w-full" isLoading={paying} disabled={!docsAccepted} onClick={handlePay}>
        ...
        {/* Standalone Pay step (Starter step 3, Premium step 6) */}
        {steps[step] === 'Pay' && (
          <div className="space-y-4">
        ...
        {!['Pay', 'Confirm', 'Docs & Pay', 'Docs & E-Sign'].includes(steps[step]) && (
        ...
        {['Pay', 'Docs & Pay', 'Docs & E-Sign'].includes(steps[step]) && step > 0 && (

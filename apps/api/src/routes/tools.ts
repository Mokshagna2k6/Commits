...
export async function toolRoutes(app: FastifyInstance) {
  // Website Audit Tool
  app.post("/tools/audit", async (req) => {
...
Return as JSON with sections: performance, seo, accessibility, security, mobile, recommendations.`;
  ...
  app.post("/tools/estimate", async (req) => {
    ...
    const items = serviceUnits.map((su) => {
      const points = su.basePoints ?? 0;
      const rate = (rateCard?.rates as any)?.[tier ?? "GROWTH"] ?? 5000;
      return { service: su.name, code: su.code, points, cost: points * rate };
    });
  ...
  app.post("/tools/brief", async (req) => {
...
Return as structured JSON.`;
  ...
  app.post("/tools/legal", async (req) => {
  ...
  app.post("/tools/invoice", async (req) => {
  ...
  app.post("/tools/express-checkout", async (req) => {
    const { serviceCode, email, phone } = req.body as { serviceCode: string; email: string; phone: string };

    const service = await prisma.serviceUnit.findFirst({ where: { code: serviceCode } });
    ...
    const order = await prisma.order.create({
      data: {
        ...
        tier: "STARTER",
        services: [service.id],
        subtotal: service.basePoints ?? 0,
        gst: Math.round((service.basePoints ?? 0) * 0.18),
        grandTotal: (service.basePoints ?? 0) + Math.round((service.basePoints ?? 0) * 0.18),
  ...
  // Preview Generator
  app.post("/tools/preview", async (req) => {
    ...
    const preview = await prisma.preview.create({
      data: { serviceId, tier: tier ?? "GROWTH", content: result },
    });
